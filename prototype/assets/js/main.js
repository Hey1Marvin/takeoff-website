/* ============================================================
   takeoff Prototyp — main.js
   Tier-Erkennung → Sternenfeld → Countdown → Scroll-FX → Mission Control
   ============================================================ */
(() => {
  "use strict";
  ...
  /* ---------- Storage: darf nie das ganze Skript killen ----------
     Safari Private Mode, Sandbox-iframes und geblockter Storage werfen
     bei jedem Zugriff. Frueher starb main.js hier in Zeile 32. */
  const store = {
    get(k) { try { return localStorage.getItem(k); } catch { return null; } },
    set(k, v) { try { localStorage.setItem(k, v); } catch { /* egal */ } },
  };
  /* Auch das JSON.parse braucht einen Guard: ein manuell verbogener
     Storage-Wert darf die Easter Eggs nicht das Skript kosten. */
  const readEggs = () => {
    try {
      const v = JSON.parse(store.get("takeoff-eggs") || "[]");
      return Array.isArray(v) ? v : [];
    } catch { return []; }
  };

  /* ---------- Event-Daten (Prototyp-Inhalt) ---------- */
  const EVENTS = [
    { id: "pwest", date: "2026-09-12T16:00:00+02:00", end: "2026-09-12T22:00:00+02:00",
      title: "Open Air: Freiräume", venue: "Bastion am Schillerplatz", city: "Potsdam West",
      genres: ["Techno", "Trance"], price: "Free", status: "ANNOUNCED",
      note: "mit Schranzverbot & Stadtjugendring" },
    { id: "marsmission", date: "2026-09-19T23:00:00+02:00", end: "2026-09-20T06:00:00+02:00",
      title: "takeoff: Marsmission", venue: "Spartacus", city: "Potsdam",
      genres: ["Trance", "Hard Trance", "Bounce"], price: "Free Entry", status: "ANNOUNCED",
      note: "Free Entry · 18+" },
    { id: "strand", date: "2026-11-14T23:00:00+01:00", end: "2026-11-15T06:00:00+01:00",
      title: "takeoff: Strandparty", venue: "Spartacus", city: "Potsdam",
      genres: ["Trance", "Hard Bounce"], price: "TBA", status: "TBA",
      note: "Arbeitstitel · Cocktails gegen den Novemberregen" },
  ];
  const nextEvent = EVENTS.find(e => new Date(e.end ?? e.date) > new Date()) ?? EVENTS[EVENTS.length - 1];

  /* ============================================================
     1 · FX-Tier  (S = statisch · M = Standard · L = volle Show)

     Das Tier wird bereits im Inline-<head>-Script gestempelt, damit es
     vor dem ersten Paint feststeht (kein FOUC). Hier wird es nur noch
     gelesen. Wichtig: `reduced` ist eine LIVE-Abfrage, keine Momentaufnahme —
     wer die Systemeinstellung waehrend der Sitzung aendert, wird sofort
     beruecksichtigt, und die User-Wahl kann sie nicht mehr aushebeln.
     ============================================================ */
  /* Boden an/aus — dritter Schalter im Mission-Control-Panel.
     Default: an, weil der Boden den Himmel erdet. */
  let ground = store.get("takeoff-ground") !== "off";
  /* Tag/Nacht gilt fuer alle Themes. Nacht ist Default — es ist eine
     Rave-Seite. */
  let day = store.get("takeoff-day") === "on";
  const dayMode = () => day;
  const groundOn = () => ground;
  const syncGroundClass = () => { html.classList.toggle("ground-on", ground); html.classList.toggle("day-mode", day); };

  /* Seiten, auf denen die Szene an den Raendern sichtbar bleiben soll.
     Im Tag-Modus steht der Inhalt sonst auf einer durchgehenden hellen
     Platte — gut fuer die Lesbarkeit, aber sie deckt eben auch den
     Marsboden samt Rover und Startrampe komplett ab. Auf Startseite,
     Eventuebersicht und Event-Detailseiten wandert die Platte deshalb auf
     die zentrierte Inhaltsspalte, links und rechts bleibt die Landschaft
     stehen. Alle uebrigen Seiten (Recht, Kontakt, Kollektiv ...) sind
     Lesestrecken und bleiben einheitlich hell. */
  {
    const file = (location.pathname.split("/").pop() || "index.html").toLowerCase();
    const edge = file === "" || file === "index.html" || file === "events.html" || /^event-/.test(file);
    html.classList.toggle("scene-edges", edge);
    /* Eventuebersicht und Event-Detailseiten. Sie bekommen im NACHTMODUS
       ausdruecklich KEINE Platte hinter dem Text — dort loest ein
       abgedunkelter Hintergrund dasselbe Problem, ohne die Seite in Kaesten
       zu zerlegen. */
    html.classList.toggle("is-event", file === "events.html" || /^event-/.test(file));
  }
  syncGroundClass();

  /* Messbetrieb: `?perf=1` legt den FPS-Watchdog still. Ohne das stuft er
     mitten in einem Messlauf herunter (er sieht ja lange Frames) und meldet
     danach Zahlen fuer eine ANDERE Effektstufe — der Vorher-Nachher-Vergleich
     misst dann zwei verschiedene Dinge. Kostet im Normalbetrieb nichts. */
  const PERF = /[?&]perf=1(?:&|$)/.test(location.search);

  const motionQuery = matchMedia("(prefers-reduced-motion: reduce)");
  const reduced = () => motionQuery.matches;
  let tier = html.dataset.fx || "m";

  /* Scroll-Fortschritt 0..1 als ZAHL. Die Sternenschleife brauchte ihn bisher
     ueber `parseFloat(html.style.getPropertyValue("--scrollp"))` zurueck — ein
     Umweg ueber Zeichenketten in jedem Bild, nur um eine Zahl zu lesen, die
     ohnehin hier entsteht. Gesetzt wird er in setProgress. */
  let scrollP = 0;

  /* Sicherheitsnetz, falls das Inline-Script fehlt oder uebergangen wurde */
  if (reduced()) { tier = "s"; html.dataset.fx = "s"; }

  /* ============================================================
     2 · Nachthimmel (Canvas 2D)

     Der Anspruch ist astronomische Plausibilitaet, nicht Deko. Vier Dinge
     machen den Unterschied zum vorherigen Feld:

     1) HELLIGKEITSVERTEILUNG. Am echten Himmel waechst die Sternzahl pro
        Magnitudenstufe um Faktor ~3: N(<m) ∝ 10^(0.48·m). Vorher waren alle
        Sterne ungefaehr gleich hell, was als Konfetti liest. Jetzt gibt es
        wenige sehr helle und sehr viele kaum sichtbare.
     2) FARBEN. Sternfarben folgen dem Planckschen Ortsbogen — die maximale
        Saettigung im gesamten Sternbereich ist ein blasses Orange, Blau ist
        nie gesaettigt. Vorher waren 23 % der Sterne kraeftig pink oder cyan.
        Zusaetzlich: das Auge sieht bei schwachen Sternen skotopisch, also
        farblos. Deshalb ist die Saettigung an die Helligkeit gekoppelt.
     3) SZINTILLATION. Luftturbulenz moduliert multiplikativ und
        unregelmaessig, nicht als gleichfoermige Sinuswelle. Drei
        inkommensurable Frequenzen pro Stern, dazu ein Hoehengradient
        (horizontnah stark, zenitnah ruhig) und seltene Helligkeitsblitze.
     4) MILCHSTRASSE. Erkennbar wird sie durch die DUNKLEN Staubbahnen davor,
        nicht durch die helle Wolke — das lassen die meisten Umsetzungen weg.

     Planeten und der Mond flimmern nicht: ihre ausgedehnte Scheibe mittelt
     die Luftunruhe weg. Das ist ein starkes, unbewusst gelesenes Signal.
     ============================================================ */
  const canvas = $("#stars");
  let starsAPI = null;
  ...
  /* Echte Mondphase aus dem Datum.
     ...
     Rueckgabe: 0 = Neumond, .25 = zunehmend halb, .5 = Vollmond,
                .75 = abnehmend halb. */
  function moonPhaseNow(date = new Date()) {
  ...
  function initStars() {
    ...
    (function loadMoonMaps() {
      ...
      grab("assets/img/moon-color.webp", "color");
      grab("assets/img/moon-height.webp", "height");
    })();
    ...
    (function loadSunMap() {
      ...
      im.src = "assets/img/sun-detail.webp";
    })();
    ...
    const GROUNDS = {
      mars:   { night: "assets/img/ground-mars-night.webp",   day: "assets/img/ground-mars-day.webp"   },
    };
    const groundImgs = {};
    ...
    function paintHorizon() {
      ...
      if (html.dataset.fx === "s" || reduced()) paintMarsFX(g, 0, scrollP || 0, 16);
    }
    ...
    function elonHead() {
      ...
      im.src = "assets/img/elon-head.webp";
      return null;
    }
    ...
    starsAPI = { resize, stop, start, refreshTheme, paintHorizon, seedGlints };
  }
  initStars();

  /* ============================================================
     3 · Lenis Smooth Scroll (nur M/L, nie bei reduced motion)

     Lenis laeuft ueber gsap.ticker statt ueber eine eigene rAF-Schleife.
     Vorher liefen zwei Loops nebeneinander (Lenis + GSAP-Ticker), was bei
     gleichzeitigem Header-Shrink und Raketen-Bewegung sichtbar jittert.
     `lagSmoothing(0)` ist Pflicht, sobald Lenis den Ticker treibt — sonst
     kollidieren die Zeitbasen und erzeugen genau das Ruckeln, das man
     vermeiden will.
     ============================================================ */
  let lenis = null;
  function startLenis() {
    if (lenis || !window.Lenis || html.dataset.fx === "s" || reduced()) return;
    lenis = new Lenis({ lerp: .11, wheelMultiplier: 1 });
    if (window.gsap) {
      gsap.ticker.add(lenisTick);
      gsap.ticker.lagSmoothing(0);
    } else {
      const loop = t => { lenis && lenis.raf(t); lenisRaf = requestAnimationFrame(loop); };
      lenisRaf = requestAnimationFrame(loop);
    }
    if (window.ScrollTrigger) {
      lenis.on("scroll", ScrollTrigger.update);
      /* Positionen neu rechnen, nachdem Lenis den Scroll-Container uebernommen hat */
  ...
  function stopLenis() {
    if (!lenis) return;
    if (window.gsap) gsap.ticker.remove(lenisTick);
  ...
  /* ============================================================
     3b · FPS-Watchdog — eigene Schleife, unabhaengig vom Sternenfeld

     Vorher haing der Watchdog im Sternen-rAF: bei Tier S lief `initStars`
     gar nicht, also gab es dort ueberhaupt keine Messung. Und jede
     manuelle Tier-Wahl schaltete ihn dauerhaft ab — auch wenn der Nutzer
     bewusst "Voll" waehlte und das Geraet danach einbrach.
     Jetzt laeuft er in jedem Tier > S und die ganze Sitzung lang, damit
     auch thermisches Drosseln auffaellt.
     ============================================================ */
  let watchdogRaf = 0;
  function startWatchdog() {
    cancelAnimationFrame(watchdogRaf);
    if (html.dataset.fx === "s" || reduced() || PERF) return;
  ...
  function downgrade() {
    ...
    try {
      const hist = JSON.parse(store.get("takeoff-fx-downgrades") || "[]")
  ...
  function applyTier(v) {
    if (v === "s") { starsAPI?.stop(); starsAPI?.resize(); stopLenis(); }
    else { if (!starsAPI) initStars(); else { starsAPI.resize(); starsAPI.start(); } startLenis(); }
    /* Die Scroll-Animationen muessen mitgezogen werden. Vorher blieben sie nach
       einem Ausflug in Stufe "Aus" fuer den Rest der Sitzung tot: der GSAP-Block
       laeuft nur einmal beim Start, und `applyTier` hat ihn nie nachgereicht. */
    buildScrollFX();
    startWatchdog();
    syncPressed();
  }
  ...
  /* Systemeinstellung aendert sich waehrend der Sitzung */
  motionQuery.addEventListener("change", () => {
    if (reduced()) { html.dataset.fx = "s"; applyTier("s"); }
  });

  /* ---------- Scroll-Progress (Neon-Faden) ---------- */
  /* scrollHeight bei jedem Scroll-Event zu lesen erzwingt ein Layout. Der Wert
     aendert sich nur bei Resize und wenn Inhalte aufklappen — also cachen. */
  let scrollMax = 0;
  ...
  /* Schub aus der Scroll-Geschwindigkeit: schneller scrollen = mehr Schub.
     Faellt weich ab, damit das Triebwerk nicht zuckt. */
  let thrust = 0, lastY = scrollY, lastTs = performance.now();

  /* GERECHNET wird pro Scroll-Ereignis, GESCHRIEBEN einmal pro Bild.
     Die beiden Dinge gehoeren getrennt:
     · Die Schubkurve muss am Ereignis haengen. Der Anstiegsfaktor .65 wirkt je
       Ereignis; wuerde man ihn auf ein Bild zusammenfassen, faellt die Spitze
       flacher aus — und genau diese Spitze ist der ganze Effekt.
     · Das Schreiben gehoert ins Bild. Scroll-Ereignisse feuern haeufiger als
       Bilder; jedes Setzen einer Custom Property auf <html> macht den ganzen
       Baum ungueltig, der `var(--scrollp)` oder `var(--thrust)` benutzt.
       Zwischenwerte, die nie gezeigt werden, kosten dabei genauso viel wie der
       letzte. Sichtbar ist nur der letzte Wert vor dem Bild — den schreiben wir,
       alle anderen entfallen. */
  let flushRaf = 0, lastSp = "", lastTh = "", lastThQ = "";
  const flushProgress = () => {
    flushRaf = 0;
    const sp = scrollMax > 0 ? (scrollP).toFixed(4) : "0";
    ...
    /* GESTUFTER Schub fuer alles, was einen Filter neu rastern laesst.
       `--thrust` liefert bis zu tausend verschiedene Werte; haengt daran ein
       `drop-shadow`, dessen Weichzeichnerradius sich mit jedem einzelnen
       aendert, wird der Schatten bei jedem Scroll-Ereignis neu gerechnet.
       In sechzehn Stufen ist ein weicher Schein auf 84 Pixel Breite nicht vom
       stufenlosen zu unterscheiden, die Neurasterungen fallen aber fast ganz
       weg. Bewegung und Deckkraft der Flamme bleiben am stufenlosen Wert —
       dort waere die Stufung sichtbar. */
    const thQ = (Math.round(thrust * 16) / 16).toFixed(3);
    if (thQ !== lastThQ) { html.style.setProperty("--thrust-q", thQ); lastThQ = thQ; }
  };
  const queueFlush = () => { if (!flushRaf) flushRaf = requestAnimationFrame(flushProgress); };

  let decayRaf = 0, lastDecay = 0;
  const setProgress = () => {
    scrollP = scrollMax > 0 ? scrollY / scrollMax : 0;
    const now = performance.now();
    const dt = Math.max(16, now - lastTs);
    const v = Math.abs(scrollY - lastY) / dt;        /* px pro ms */
    lastY = scrollY; lastTs = now;
    /* Asymmetrisch: der Anstieg holt in ein bis zwei Frames fast den ganzen
       Weg auf, das Abklingen braucht rund eine halbe Sekunde. Genau darin
       besteht das Gefuehl von Schub — symmetrisches Glaetten (frueher eine
       CSS-transition ueber .5s) macht daraus ein Schwellen, das man nicht
       bemerkt. Der frueher hier stehende Mindestwert .22 ist bewusst weg: ohne
       ruhigen Ausgangszustand gibt es nichts, wogegen die Spitze kontrastiert. */
    const target = Math.min(1, v / 1.4);
    thrust += (target - thrust) * (target > thrust ? .65 : .10);
    queueFlush();
    startDecay();
  };
  ...
  /* Schub klingt auch ohne weitere Scroll-Ereignisse ab.
     Frueher lief dafuer ein setInterval die ganze Sitzung mit. Jetzt startet
     die Schleife erst beim Scrollen und beendet sich selbst, sobald der Schub
     unten ist. Der Takt bleibt derselbe: ein Schritt je 120 ms, damit die
     Abklingkurve unveraendert ist. Bewusst NICHT in die Sternen-Schleife
     gefaltet — die laeuft in Stufe "Aus" gar nicht, und dann bliebe der Schub
     fuer den Rest der Sitzung eingefroren stehen. */
  function startDecay() {
    if (decayRaf) return;
    lastDecay = performance.now();
    decayRaf = requestAnimationFrame(stepDecay);
  }
  function stepDecay(t) {
    decayRaf = 0;
    let changed = false;
    while (t - lastDecay >= 120) {
      lastDecay += 120;
      if (thrust > .01) { thrust *= .90; changed = true; }
    }
    if (changed) queueFlush();
    if (thrust > .01) decayRaf = requestAnimationFrame(stepDecay);
  }


  /* ---------- Dauerschleifen anhalten, sobald sie aus dem Bild sind ----------
     `sheen` auf der Wortmarke und `shimmer` auf den Fortschrittsbalken
     animieren `background-position`. Das ist keine Eigenschaft, die der
     Compositor allein bewegen kann — die Flaeche wird in jedem Bild neu
     gezeichnet. Die Wortmarke traegt dabei zusaetzlich `background-clip: text`,
     `-webkit-text-stroke` und einen `drop-shadow`; ihr Glyphenlayer muss also
     jedes Mal neu gerastert werden, und sie ist die groesste Textflaeche der
     Startseite.
     Beides lief bisher die ganze Sitzung durch — auch lange, nachdem das
     Element aus dem Bild gescrollt war. Was niemand sieht, muss nicht laufen;
     angehalten wird ueber `animation-play-state`, die Schleife setzt beim
     Wiedereintritt genau dort fort, wo sie stand. Der Rand von 120 px sorgt
     dafuer, dass sie schon laeuft, bevor das Element sichtbar wird. */
  if ("IntersectionObserver" in window) {
    const idleObs = new IntersectionObserver(es => es.forEach(e => {
      e.target.classList.toggle("anim-idle", !e.isIntersecting);
    }), { rootMargin: "120px" });
    $$(".wordmark, .tta .bar, .band-track").forEach(el => idleObs.observe(el));
  }

  /* ============================================================
     3c · Navigation

     Das Overlay ist ein <dialog>: showModal() bringt Focus-Trap, Escape und
     die richtige ARIA-Rolle mit, statt dass wir das nachbauen. Der Top-Layer
     liegt ausserdem ausserhalb jeder Containing-Block-Kette — dadurch springen
     die fixierten Elemente (Faden, Sticky-CTA) beim Oeffnen nicht.
     ============================================================ */
  const topbar = $(".topbar");
  const menu = $("#menu");
  const burger = $(".nav-burger");

  /* -- Telemetrie faehrt ein, und die Leiste meldet ihre Hoehe nach oben --

     Zwei Dinge passieren hier, und sie haengen zusammen.

     1. HYSTERESE statt einer Schwelle. Vorher stand hier ein einzelnes
        `scrollY > 120`. An genau diesem Punkt flackert der Zustand am
        Trackpad-Gummiband und beim iOS-Overscroll hin und her, und jeder
        Wechsel kostet einen Layoutsprung. Einfahren ab 96, ausfahren erst
        wieder unter 48 — die 48px Totzone entsprechen rund einer Zeilenhoehe
        Scroll. Bewusst KEINE Richtungslogik ("faehrt beim Hochscrollen aus"):
        das ist die haeufigste Ursache springender Kopfleisten, weil jede
        versehentliche Richtungsumkehr einen Wechsel ausloest.

     2. --topbar-cur. Custom Properties vererben abwaerts, nicht aufwaerts —
        :root kann die Hoehe der Leiste nicht selbst kennen. Diese eine Zeile
        spiegelt sie nach oben, und ab da lesen ALLE dieselbe Quelle:
        scroll-padding-top, die Hero-Hoehe und headerOffset() weiter unten.
        Frueher rechnete CSS starr 100px, waehrend das Skript live mass — nach
        dem Einfahren schoss jeder Ankersprung um rund 48px ueber.

        Gemessen wird am ENDE des Uebergangs, nicht pro Bild: ein
        offsetHeight-Lesen pro Bild erzwingt ein Layout auf einem klebenden
        Element, also fuer den ganzen Kopfbereich. Genau davor warnen die
        Kommentare bei .nav-progress und .rocket. */
  if (topbar) {
    const setTopbarH = () =>
      html.style.setProperty("--topbar-cur", topbar.offsetHeight + "px");

    let flying = false;
    const syncTopbar = () => {
      const should = flying ? scrollY > 48 : scrollY > 96;
      if (should === flying) return;
      flying = should;
      topbar.classList.toggle("is-flying", should);
    };
    addEventListener("scroll", syncTopbar, { passive: true });

    /* Am Ende des Hoehen-Uebergangs nachmessen. Der Sicherheits-Timeout
       greift, wenn die Transition gar nicht laeuft (reduzierte Bewegung). */
    const hud = topbar.querySelector(".hud");
    let hTimer = 0;
    const remeasure = () => { clearTimeout(hTimer); hTimer = setTimeout(setTopbarH, 400); };
    hud?.addEventListener("transitionend", e => {
      if (e.propertyName === "height") setTopbarH();
    });
    addEventListener("scroll", remeasure, { passive: true });
    addEventListener("resize", setTopbarH, { passive: true });

    syncTopbar();
    setTopbarH();
    document.fonts?.ready.then(setTopbarH);
    window.takeoffMeasureTopbar = setTopbarH;   /* fuer spaeter eingefuegte Teile */
  }
  ...
  if (menu && burger && typeof menu.showModal === "function") {
    ...
    const openMenu = () => {
      ...
      lenis?.stop();                       /* nicht overflow:hidden — das wuerde
                                              Lenis' Scrollposition zerreissen */
      menu.showModal();
      burger.setAttribute("aria-expanded", "true");
      burger.setAttribute("aria-label", "Menü schließen");
      menu.querySelector(".menu-list a")?.focus();
    };
    const closeMenu = () => {
      if (!menu.open) return;
      menu.close();
    ...
    /* Klick auf den Backdrop schliesst ebenfalls */
    ...
    menu.addEventListener("close", () => {
      ...
      lastFocus?.focus();                  /* Fokus zurueck auf den Ausloeser —
                                              sonst faengt man oben wieder an */
    });
  }

  /* -- Ankersprünge: Ziel darf nicht unter der Topbar landen --
     `scroll-padding-top` im CSS deckt den Fall ohne JS ab; mit Lenis muss der
     Versatz explizit mitgegeben werden. Das Vorzeichen ist hier nachgemessen,
     nicht aus der Doku uebernommen. */
  /* Liest DIESELBE Property, die CSS benutzt — nicht mehr eine eigene Messung.
     Damit sind der CSS- und der JS-Weg per Konstruktion identisch statt per
     Absprache. Der Zuschlag kommt aus --bar-gap, damit auch er nur einmal
     existiert. */
  ...
  const headerOffset = () =>
    topbar ? cssPx("--topbar-cur", topbar.offsetHeight) + cssPx("--bar-gap", 14) : 0;
  $$('a[href*="#"]').forEach(a => {
    const url = new URL(a.href, location.href);
    if (!url.hash) return;
    /* Nicht die Pfade vergleichen: die Links heissen "index.html#…", die Seite
       wird aber unter "/" ausgeliefert. Der Pfadvergleich schlug deshalb fehl
       und die Seite lud bei jedem Navigationsklick komplett neu, statt sanft zu
       scrollen. Entscheidend ist allein, ob das Sprungziel hier existiert. */
    a.addEventListener("click", e => {
      let target = null;
      try { target = document.querySelector(url.hash); } catch { /* ungueltiger Selektor */ }
      if (!target) return;
      e.preventDefault();
  ...
  /* -- Aktive SEITE in der Navigation --
     Die Navigation wechselt Seiten, nicht Abschnitte (events.html, artists.html,
     kollektiv.html, awareness.html). Vorher zeigte sie auf Anker der Startseite,
     die Unterseiten waren dadurch von dort gar nicht erreichbar. Damit faellt
     auch die Abschnittsmarkierung weg — an ihre Stelle tritt die Markierung der
     aktuellen Seite, was bei einer Seitennavigation das Richtige ist. */
  {
  ...
  /* -- Aktiver Abschnitt, falls eine Seite doch Anker-Navigation hat -- */
  ...
  if (navLinks.length && "IntersectionObserver" in window) {
    ...
    const io = new IntersectionObserver(entries => {
      entries.forEach(en => { if (en.isIntersecting) current = en.target; });
      navLinks.forEach(a => a.removeAttribute("aria-current"));
      if (current && byId.has(current)) byId.get(current).setAttribute("aria-current", "true");
    }, { rootMargin: "-45% 0px -50% 0px" });
    byId.forEach((_, sec) => io.observe(sec));
  }

  /* ============================================================
     4 · Countdown  (T-MINUS zum nächsten Event)
     ============================================================ */
  const clock = $("#tminus-clock");
  if (clock) {
    const target = new Date(nextEvent.date).getTime();
    const pad = n => String(n).padStart(2, "0");
    const tick = () => {
      let d = target - Date.now();
      if (d <= 0) { clock.innerHTML = "LIFTOFF <small>· läuft!</small>"; return; }
      const days = Math.floor(d / 864e5);
      const hrs = Math.floor(d / 36e5) % 24;
      const min = Math.floor(d / 6e4) % 60;
      const sec = Math.floor(d / 1e3) % 60;
      clock.innerHTML = `${days}<small>d</small> ${pad(hrs)}<small>h</small> ${pad(min)}<small>m</small> ${pad(sec)}<small>s</small>`;
    };
    tick(); setInterval(tick, 1000);
  }

  /* ============================================================
     4b · Fit-to-Width für Wortmarke & Event-Titel
     Planet Kosmos darf NIE umbrechen oder abgeschnitten werden —
     wir messen und verkleinern exakt auf die verfügbare Breite.
     ============================================================ */
  const MIN_FIT = 26;
  function fitText(el) {
    if (!el) return;
    ...
    if (!parent) return;
    /* Gegen die *innere* Breite des Containers messen, nicht gegen dessen
       clientWidth samt Padding — sonst wird bei gepolsterten Eltern zu gross
       gerechnet. Der Container hat jetzt eine inhaltsunabhaengige Breite,
       damit ist die Messung nicht mehr zirkulaer. */
    const pcs = getComputedStyle(parent);
    const avail = parent.clientWidth - parseFloat(pcs.paddingLeft) - parseFloat(pcs.paddingRight);
    if (avail < 40) return;                       /* Viewport noch 0 (versteckter Tab) */

    el.style.fontSize = "";                       /* zurück auf CSS-clamp als Obergrenze */
    el.classList.remove("is-cramped");
    const base = parseFloat(getComputedStyle(el).fontSize);
    const needed = el.scrollWidth;
    if (needed <= avail) return;

    const fitted = base * avail / needed;
    if (fitted >= MIN_FIT) { el.style.fontSize = fitted + "px"; return; }

    /* Selbst bei Mindestgroesse zu breit: dann lieber umbrechen lassen als
       abschneiden. Vorher lief `white-space: nowrap` hier ueber und wurde von
       `body { overflow-x: hidden }` stillschweigend gekappt. */
    el.style.fontSize = MIN_FIT + "px";
    el.classList.add("is-cramped");
  }
  const fitAll = () => { fitText($(".wordmark")); fitText($(".etitle")); };
  fitAll();
  ...
  /* ============================================================
     5 · Reveals & Scroll-Choreografie
     GSAP wenn vorhanden (M/L), sonst IntersectionObserver-Fallback
     ============================================================ */
  const fxOn = () => html.dataset.fx !== "s";
  /* Voll = alles, Normal = nur die ruhigen Reveals. Bisher unterschied GSAP
     zwischen den beiden Stufen ueberhaupt nicht. */
  const fxFull = () => html.dataset.fx === "l";

  /* Die Choreografie lebt in einem gsap.context, damit sie sich sauber
     zuruecknehmen und neu aufbauen laesst. Ohne das blieben die Animationen
     nach einem Ausflug in Stufe "Aus" fuer den Rest der Sitzung tot. */
  let scrollCtx = null;
  ...
  function buildScrollFX() {
    ...
    if (!fxOn() || !window.gsap || !window.ScrollTrigger) {
      /* Ohne GSAP oder in Stufe "Aus": alles sofort sichtbar, kein Beobachter */
      if (!fxOn()) { $$(".reveal").forEach(el => el.classList.add("in")); return; }
    ...
    scrollCtx = gsap.context(() => {
      /* Hero-Intro laeuft nur beim ersten Aufbau — beim Stufenwechsel soll der
         Hero nicht erneut hereinfliegen. */
      if (!heroPlayed) {
        heroPlayed = true;
        gsap.timeline({ defaults: { ease: "expo.out" } })
          .from(".wordmark", { y: 60, opacity: 0, duration: 1.1 })
          .from(".hero .pretitle", { opacity: 0, y: 10, duration: .6 }, "-=.7")
          .from(".hero .tagline", { opacity: 0, y: 14, duration: .6 }, "-=.45")
          .from(".tminus", { opacity: 0, scale: .94, duration: .55 }, "-=.35")
          .from(".next-card", { opacity: 0, y: 30, duration: .7 }, "-=.3")
          .from(".scroll-hint", { opacity: 0, duration: .5 }, "-=.2");
      }

      /* Section-Reveals (beide Stufen) */
      $$(".reveal").forEach(el => {
        if (el.classList.contains("in")) return;      /* schon aufgedeckt */
      ...
      /* `immediateRender: false` ist beim Wiederaufbau entscheidend: sonst
         setzt gsap.from() den Startzustand (opacity 0) sofort, auch fuer
         Elemente, deren Trigger bereits vorbei ist — die blieben dann
         unsichtbar haengen. */
      const fromCfg = { immediateRender: false };

      $$(".card-grid, .crewgrid, .setgrid").forEach(grid => {
        gsap.from(grid.children, {
          ...fromCfg, y: 34, opacity: 0, duration: .8, ease: "expo.out",
          stagger: .09,
          scrollTrigger: { trigger: grid, start: "top 84%", once: true },
        });
      });

      gsap.utils.toArray(".flog li").forEach(li => {
        gsap.from(li, {
          ...fromCfg, x: -26, opacity: 0, duration: .7, ease: "expo.out",
      ...
      const lineup = $(".lineup");
      if (lineup) {
        gsap.from(lineup.children, {
          ...fromCfg, y: 26, opacity: 0, duration: .6, ease: "back.out(1.6)",
          stagger: .08,
          scrollTrigger: { trigger: lineup, start: "top 85%", once: true },
        });
      }

      /* Ab hier nur Stufe "Voll": alles, was durchgehend scrubbt */
      if (!fxFull()) return;

      /* Nur noch Parallaxe, kein Wegblenden mehr: in .hero-inner liegen
         Wortmarke, Tagline, Countdown UND die Open-Air-Karte. Das frühere
         `opacity: .35` liess genau die Inhalte im Hintergrund versinken,
      ...
      $$(".ditem").forEach(item => {
        gsap.to(item, {
          y: parseFloat(item.dataset.spd || -50), ease: "none",
  ...
  /* ---------- Karten-Tilt + Hover-Licht (nur L, nur Pointer fein) ---------- */
  if (matchMedia("(pointer: fine)").matches) {
    $$(".mcard").forEach(card => {
      card.addEventListener("pointerenter", () => {
        /* Waehrend des Tiltens keine Transition: sonst laeuft jede
           Mausbewegung 350 ms hinterher und fuehlt sich traege an. */
        if (html.dataset.fx === "l" && !reduced()) card.style.transition = "none";
      });
      card.addEventListener("pointermove", e => {
        const r = card.getBoundingClientRect();
        card.style.setProperty("--mx", ((e.clientX - r.left) / r.width) * 100 + "%");
        card.style.setProperty("--my", ((e.clientY - r.top) / r.height) * 100 + "%");
        if (html.dataset.fx !== "l" || reduced() || card.classList.contains("open")) return;
        const rx = ((e.clientY - r.top) / r.height - .5) * -6;
        const ry = ((e.clientX - r.left) / r.width - .5) * 6;
        card.style.transform = `translateY(-6px) rotateX(${rx}deg) rotateY(${ry}deg)`;
      });
      card.addEventListener("pointerleave", () => {
        /* Transition zurueckgeben, DANN zuruecksetzen — sonst springt die
           Karte hart in die Ausgangslage statt sanft zurueckzukippen. */
  ...
  /* Der Liquid-Wobble der Wortmarke ist entfallen — mit ihm der SVG-Filter in
     index.html und event-marsmission.html. Der feDisplacementMap sampelte
     ausserhalb der Quelle und machte den Glyphenrand echt halbtransparent;
     die Marke soll aber unter allen Umstaenden deckend stehen. */

  /* ---------- Set-Karten: Zwei-Klick-Demo ---------- */
  /* ---------- Aufklappbare Karten (Missionen, Artists) ---------- */
  $$("[data-expand]").forEach(card => {
    const btn = $(".m-toggle", card);
    const set = open => {
      card.classList.toggle("open", open);
      if (open) card.style.transform = "";               /* Tilt-Rest entfernen */
      btn?.setAttribute("aria-expanded", String(open));
    };
    card.addEventListener("click", e => {
      /* Links & Buttons im aufgeklappten Teil nicht kapern */
      if (e.target.closest("a") || e.target.closest(".m-more button")) return;
      const open = !card.classList.contains("open");
      /* Accordion: Geschwister im selben Grid schliessen */
      card.parentElement?.querySelectorAll("[data-expand].open").forEach(o => {
        if (o !== card) { o.classList.remove("open"); $(".m-toggle", o)?.setAttribute("aria-expanded", "false"); }
      });
      set(open);
    });
  });
  ...
  /* ---------- Versteckte Sammel-Items (1 pro Theme, Fortschritt lokal) ---------- */
  const secrets = $$(".ditem[data-secret]");
  if (secrets.length) {
    ...
    secrets.forEach(item => item.addEventListener("click", () => {
      const found = new Set(readEggs());
      const isNew = !found.has(item.dataset.secret);
      if (isNew) {
        found.add(item.dataset.secret);
        store.set("takeoff-eggs", JSON.stringify([...found]));
      }
      item.classList.add("found");
      setTimeout(() => item.classList.remove("found"), 950);
      showToast(isNew
        ? (found.size === 3 ? "🦄 Alle 3 Verstecke gefunden — Crew-Material!" : `✨ Versteck gefunden — ${found.size}/3 Themes`)
        : "Schon entdeckt 🤫 — wechsel mal das Theme …");
    }));
  }

  /* ============================================================
     6 · Mission Control  (FX-Tier + Theme umschalten, persistiert)
     ============================================================ */
  function syncPressed() {
    const theme = html.dataset.theme || "space";
    /* Space ist wieder reiner Nachthimmel — dort gibt es keinen Boden mehr,
       den der Schalter umlegen koennte. Er wird deaktiviert, nicht entfernt:
       die Buttons haengen an einem generischen Handler, bedingtes Markup ueber
       sechs Dateien waere unverhaeltnismaessig, und ein Bedienelement, das
       beim Themewechsel verschwindet, laesst das Panel springen. */
    const noGround = theme === "space";
    $$("[data-set-fx]").forEach(b => b.setAttribute("aria-pressed", b.dataset.setFx === html.dataset.fx));
    $$("[data-set-theme]").forEach(b => b.setAttribute("aria-pressed", theme === b.dataset.setTheme));
    $$("[data-set-ground]").forEach(b => {
      b.setAttribute("aria-pressed", (b.dataset.setGround === "on") === ground);
      b.disabled = noGround;
    });
    /* "toggle" ist der eine Knopf in der Kopfleiste: er traegt den Zustand
       selbst, waehrend "night"/"day" im Panel und im Menue je eine Seite der
       Wahl darstellen. Alle drei haengen weiterhin an EINEM Vertrag und
       werden von EINEM syncPressed() versorgt. */
    $$("[data-set-day]").forEach(b => b.setAttribute("aria-pressed",
      b.dataset.setDay === "toggle" ? String(day)
                                    : String((b.dataset.setDay === "day") === day)));
    const row = $("[data-set-ground]")?.closest(".row");
    if (row) row.classList.toggle("is-off", noGround);
  }
  $$("[data-set-fx]").forEach(btn => btn.addEventListener("click", () => {
    const v = btn.dataset.setFx;
    /* Die User-Wahl darf reduced-motion NICHT aushebeln — vorher konnte
       ein einmaliges Klicken auf "Voll" alle GSAP-Tweens und das Canvas
       dauerhaft gegen die Systemeinstellung erzwingen. */
    if (reduced() && v !== "s") {
      store.set("takeoff-fx", "s");
      syncPressed();
      return;
    }
    html.dataset.fx = v;
    store.set("takeoff-fx", v);
    applyTier(v);
  }));
  $$("[data-set-ground]").forEach(btn => btn.addEventListener("click", () => {
    ground = btn.dataset.setGround === "on";
    store.set("takeoff-ground", ground ? "on" : "off");
    syncGroundClass();
    starsAPI?.paintHorizon();
    starsAPI?.seedGlints();
    syncPressed();
  }));
  $$("[data-set-day]").forEach(btn => btn.addEventListener("click", () => {
    day = btn.dataset.setDay === "toggle" ? !day : btn.dataset.setDay === "day";
    store.set("takeoff-day", day ? "on" : "off");
    syncGroundClass();
    /* Voller Neuaufbau ueber refreshTheme() -> resize(). Frueher genuegte
       hier paintHorizon() + seedGlints(), weil der Tag/Nacht-Schalter nichts
       an dem aenderte, was paintBackdrop() zeichnet. Seit es einen
       Tag-Himmel gibt, stimmt das nicht mehr: die statische Ebene traegt
       nachts Milchstrasse und Sterne und tagsueber Himmelsverlauf und
       Wolken, und die Sternenliste selbst wird tagsueber gar nicht erst
       erzeugt. Beides entsteht in resize(). paintHorizon() und seedGlints()
       ruft refreshTheme() mit auf. */
    starsAPI?.refreshTheme();
    syncPressed();
  }));
  $$("[data-set-theme]").forEach(btn => btn.addEventListener("click", () => {
    ...
    if (!document.body.dataset.themeLock) store.set("takeoff-theme", v);
    starsAPI?.refreshTheme();
    syncPressed();
  }));
  /* Theme aus Storage (außer die Seite erzwingt eins, z. B. Event-Seite) */
  if (!document.body.dataset.themeLock) {
    const th = store.get("takeoff-theme");
    if (th && th !== "space") html.dataset.theme = th;
  }
  syncPressed();
  starsAPI?.refreshTheme();
})();
