...
| **Qualität wird stufenlos geregelt**, nicht in Sprüngen. Schwellwerte immer **relativ zur gemessenen Bildwiederholrate** — feste Millisekunden stufen 120-Hz-Geräte falsch ein und lesen den 30-fps-Deckel des Stromsparmodus als Dauerüberlast (daran ging der alte Watchdog zugrunde). Die Rate kommt aus dem 10. Perzentil eines gleitenden Fensters, **nicht** einmalig aus den ersten Bildern: die fallen in die Ladephase, und dann wird der Regler umso nachsichtiger, je schlechter eine Seite startet | `src/lib/sky/qualitaet.ts` |
| **Was bei Knappheit zuerst fällt, steht in der Sparleiter** (`data-spar` 0…5) und NIRGENDS sonst. Reihenfolge von Marvin: 1. Hero-Video · 2. Sternen-Animation · 3. Sternenzahl und Themes. Neue teure Effekte dort einhängen (`:root[data-spar="5"] …` bzw. `sparStufe()`), statt eigene Schwellen zu erfinden — sonst hat die Seite wieder fünf Meinungen darüber, was Sparen heißt | `src/lib/sky/qualitaet.ts`, Konstante `SPAR_RUNTER` |
| **Die Automatik muss zurückfinden.** Sie darf herunterstufen, aber sie muss es von selbst zurücknehmen — als Versuch mit wachsendem Abstand, nie als Schlussfolgerung (in Stufe „s" ist die Seite leer, dort beweist „viel Luft" nichts). Und sie schreibt **nichts** dauerhaft: `takeoff-fx` ist der Startwert, keine Decke. Genau die Sticky-Mechanik machte die Herunterstufung früher unumkehrbar. Zwei Sperren überstimmt sie nicht, weil beide keine Leistungsfragen sind: `prefers-reduced-motion` und der Datensparmodus | `src/lib/sky/qualitaet.ts`, `stufeHoch()` |
